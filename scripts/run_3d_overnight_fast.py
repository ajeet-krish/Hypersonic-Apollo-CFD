...
Uses the working draft mesh (2.7M elements) and runs a 4-stage Mach ramp.
Skips expensive Netgen optimization for faster mesh generation.
...
Usage:
    uv run python scripts/run_3d_overnight_fast.py
"""
import sys
import time
import math
...
# Mesh settings (draft tier, mm) - faster generation
MIN_ELEMENT_SIZE = 50.0   # mm near body
MAX_ELEMENT_SIZE = 2000.0 # mm farfield
...
def generate_mesh() -> Path:
    """Generate draft-tier 3D mesh (fast, no Netgen optimization)."""
    print("\n" + "="*60)
    print("  STEP 1: Generate Draft-Tier 3D Mesh (fast)")
    print("="*60)
    ...
    gmsh.model.add("wind_tunnel_fast")

    # Create body from contour (50 points for speed)
    n = 50
    ...
    # Mesh generation (NO Netgen optimization for speed)
    print(f"  Meshing (min={MIN_ELEMENT_SIZE}mm, max={MAX_ELEMENT_SIZE}mm)...")
    gmsh.option.setNumber("Mesh.Algorithm3D", 1)
    gmsh.option.setNumber("Mesh.Smoothing", 10)  # Reduced from 20
    t0 = time.time()
...
def write_su2_config(stage: dict, workdir: Path, is_restart: bool = False) -> Path:
...
def run_stage(stage: dict, workdir: Path, mesh_path: Path, is_restart: bool = False) -> bool:
    ...
    # Write config
    write_su2_config(stage, workdir, is_restart)
...
def main():
    ...
    print(f"  Mesh: Draft tier (fast, no Netgen)")
    ...
    for stage in STAGES:
        ...
        if history_path.exists():
            ...
            if len(lines) > 2:
                ...
                iters = len(lines) - 1
                print(f"  {stage['name']:20s}: {iters:5d} iters, RMS={rms:.4f}")
        else:
