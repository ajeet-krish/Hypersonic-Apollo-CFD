...
class TestConvergenceStage:
    ...
    def test_custom_values(self):
        ...
        assert stage.conv_method == "AUSM"
        assert stage.linear_solver == "FGMRES"
        assert stage.linear_solver_error == pytest.approx(1e-6)
...
class TestConvergenceStrategyForMach:
    ...
    Thresholds:
        M <= 2:    single stage (direct solve)
        2 < M <= 5:  two stages (M=2 ramp)
        5 < M <= 10: three stages (M=2 -> M=5 ramp)
        M > 10:    four stages (M=2 -> M=5 -> M=10 ramp)
    """
    ...
    def test_mach_2_boundary_single(self):
        s = ConvergenceStrategy.for_mach(2.0)
        assert len(s.stages) == 1

    def test_single_stage_mach_matches_target(self):
        s = ConvergenceStrategy.for_mach(1.5)
        assert s.stages[0].mach == pytest.approx(1.5)

    # --- Two stages (2 < M <= 5) ---

    def test_mach_3_two_stage(self):
        s = ConvergenceStrategy.for_mach(3.0)
        assert len(s.stages) == 2

    def test_mach_4_two_stage(self):
        s = ConvergenceStrategy.for_mach(4.0)
        assert len(s.stages) == 2

    def test_mach_5_boundary_two(self):
        s = ConvergenceStrategy.for_mach(5.0)
        assert len(s.stages) == 2

    def test_two_stage_first_mach_is_2(self):
        s = ConvergenceStrategy.for_mach(4.0)
        assert s.stages[0].mach == pytest.approx(2.0)

    def test_two_stage_last_mach_is_target(self):
        s = ConvergenceStrategy.for_mach(4.0)
        assert s.stages[1].mach == pytest.approx(4.0)

    def test_two_stage_mach_monotonic(self):
        s = ConvergenceStrategy.for_mach(3.5)
        machs = [st.mach for st in s.stages]
        assert machs == sorted(machs)

    # --- Three stages (5 < M <= 10) ---

    def test_mach_6_three_stage(self):
        s = ConvergenceStrategy.for_mach(6.0)
        assert len(s.stages) == 3

    def test_mach_8_three_stage(self):
        s = ConvergenceStrategy.for_mach(8.0)
        assert len(s.stages) == 3

    def test_mach_10_boundary_three(self):
        s = ConvergenceStrategy.for_mach(10.0)
        assert len(s.stages) == 3

    def test_three_stage_first_mach_is_2(self):
        s = ConvergenceStrategy.for_mach(8.0)
        assert s.stages[0].mach == pytest.approx(2.0)

    def test_three_stage_second_mach_is_5(self):
        s = ConvergenceStrategy.for_mach(8.0)
        assert s.stages[1].mach == pytest.approx(5.0)

    def test_three_stage_last_mach_is_target(self):
        s = ConvergenceStrategy.for_mach(8.0)
        assert s.stages[2].mach == pytest.approx(8.0)

    # --- Four stages (M > 10) ---

    def test_mach_10_001_four_stage(self):
        s = ConvergenceStrategy.for_mach(10.001)
        assert len(s.stages) == 4

    def test_mach_15_6_four_stage(self):
        s = ConvergenceStrategy.for_mach(15.6)
        assert len(s.stages) == 4

    def test_mach_25_four_stage(self):
        s = ConvergenceStrategy.for_mach(25.0)
        assert len(s.stages) == 4

    def test_four_stage_first_mach_is_2(self):
        s = ConvergenceStrategy.for_mach(15.6)
        assert s.stages[0].mach == pytest.approx(2.0)

    def test_four_stage_second_mach_is_5(self):
        s = ConvergenceStrategy.for_mach(15.6)
        assert s.stages[1].mach == pytest.approx(5.0)

    def test_four_stage_third_mach_is_10(self):
        s = ConvergenceStrategy.for_mach(15.6)
        assert s.stages[2].mach == pytest.approx(10.0)

    def test_four_stage_last_mach_is_target(self):
        s = ConvergenceStrategy.for_mach(15.6)
        assert s.stages[3].mach == pytest.approx(15.6)

    # --- Invariants across all Mach ranges ---

    def test_stage_mach_progression_15_6(self):
        s = ConvergenceStrategy.for_mach(15.6)
        machs = [st.mach for st in s.stages]
        assert machs == sorted(machs)
        assert machs[-1] == pytest.approx(15.6)

    def test_stage_mach_progression_8(self):
        s = ConvergenceStrategy.for_mach(8.0)
        machs = [st.mach for st in s.stages]
        assert machs == sorted(machs)
        assert machs[-1] == pytest.approx(8.0)

    def test_stage_mach_progression_4(self):
        s = ConvergenceStrategy.for_mach(4.0)
        machs = [st.mach for st in s.stages]
        assert machs == sorted(machs)
        assert machs[-1] == pytest.approx(4.0)
    ...
    def test_all_stages_conv_method_roe(self):
        s = ConvergenceStrategy.for_mach(15.6)
        for st in s.stages:
            assert st.conv_method == "ROE"
...
class TestConvergenceStrategyMachRamp:
    ...
    def test_ramp_all_stages_same_tolerance(self):
        s = ConvergenceStrategy.mach_ramp(20.0, n_stages=3)
        for st in s.stages:
            assert st.linear_solver_error == pytest.approx(1e-4)

    def test_ramp_all_stages_same_linear_iterations(self):
        s = ConvergenceStrategy.mach_ramp(20.0, n_stages=3)
        for st in s.stages:
            assert st.linear_solver_iter == 50
    ...
    def test_ramp_linear_solver_defaults(self):
        s = ConvergenceStrategy.mach_ramp(20.0, n_stages=3)
        for st in s.stages:
            assert st.linear_solver_error == pytest.approx(1e-4)
            assert st.linear_solver_iter == 50
            assert st.cfl_adapt_increase == pytest.approx(1.2)
...
class TestConvergenceEdgeCases:
    ...
    def test_very_high_mach_four_stage(self):
        s = ConvergenceStrategy.for_mach(100.0)
        assert len(s.stages) == 4
        assert s.stages[-1].mach == pytest.approx(100.0)

    def test_exact_boundary_mach_2(self):
        s = ConvergenceStrategy.for_mach(2.0)
        assert len(s.stages) == 1
        # Just above boundary -> two stages
        s2 = ConvergenceStrategy.for_mach(2.0 + 1e-9)
        assert len(s2.stages) == 2

    def test_exact_boundary_mach_5(self):
        s = ConvergenceStrategy.for_mach(5.0)
        assert len(s.stages) == 2
        # Just above boundary -> three stages
    ...
    def test_exact_boundary_mach_10(self):
        s = ConvergenceStrategy.for_mach(10.0)
        assert len(s.stages) == 3
        # Just above boundary -> four stages
        s2 = ConvergenceStrategy.for_mach(10.0 + 1e-9)
        assert len(s2.stages) == 4
